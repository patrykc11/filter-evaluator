...
def sample_images(batches_done):
    ...
    real_X = imgs['X'].cuda() if cuda else imgs['X']
    fake_Y = G(real_X)
    real_Y = imgs['Y'].cuda() if cuda else imgs['Y']
    fake_X = F(real_Y)
    # Save sample images
    # Example: save_image(fake_Y, 'images/fake_Y_%d.png' % batches_done, nrow=5, normalize=True)
    pass

def save_checkpoint(epoch, batch, G, F, D_X, D_Y, optimizer_G, optimizer_D_X, optimizer_D_Y, checkpoint_dir='checkpoints'):
    ...
    }, os.path.join(checkpoint_dir, f'checkpoint_epoch_{epoch}_batch_{batch}.pth'))
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(ImageDataset('images_subs', transforms_=transform), batch_size=1, shuffle=True, num_workers=8)
